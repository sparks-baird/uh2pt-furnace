...
# and `pip install uh2pt-furnace` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "uh2pt-furnace", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n uh2pt-furnace -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "uh2pt-furnace", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "uh2pt-furnace", "YOUR_CONSOLE_SCRIPT"]
